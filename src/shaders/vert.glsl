...
layout (location = 3) in int   aOverlayTextureID;
layout (location = 4) in int   aColourmapX;
layout (location = 5) in int   aColourmapY;
layout (location = 6) in int   aFlags;

out vec3  normal;
out float textureID;
out float overlayTextureID;
out vec2  texCoord;
flat out int flags;
...
void main() {
    ...
    overlayTextureID = float(aOverlayTextureID);
